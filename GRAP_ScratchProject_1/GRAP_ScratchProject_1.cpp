...
int main(void)
{
    ...
    window = glfwCreateWindow(width, height, "Red Circle at Origin", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
        return -1;
    ...
    // Physics world and single render particle
    Physics::PhysicsWorld world;
    Physics::Particle* p = new Physics::Particle();
    p->Position = Physics::MyVector(-500.0f, 0.0f, 0.0f);
    p->mass = 0.9f;
	p->AddForce(Physics::MyVector(500.0f, 2000.0f, 0.0f));
    world.AddParticle(p);

    Physics::RenderParticle* rp = new Physics::RenderParticle(p, sphereModel, Physics::MyVector(1, 0, 0)); 
